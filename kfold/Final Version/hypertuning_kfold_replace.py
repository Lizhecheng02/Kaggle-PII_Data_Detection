# export KFOLD=1
# wandb sweep --project PII hypertuning_kfold_replace.yaml
...
# 默认为0，如果环境变量未设置
fold = int(os.getenv('KFOLD', 0))

print(f"fold {fold}")
# load dataset
valid_df = pd.read_json(
    f"../kaggle_dataset/replace/test_fold_{fold}_replace.json")

train1 = pd.read_json(
    f"../kaggle_dataset/replace/train_fold_{fold}_replace.json")
train2 = pd.read_json(
    "../kaggle_dataset/replace/nb_mixtral-8x7b-v1_replace.json")
train3 = pd.read_json(
    "../kaggle_dataset/replace/rewrite_1k_more_name_en_name_replace.json")
# train3 = train3.sample(frac=0.5, random_state=777).reset_index(drop = True)

train4 = pd.read_json(
    "../kaggle_dataset/replace/Fake_data_same_nb_1000_0_replace.json")
# tmp_train = pd.read_json("../kaggle_dataset/Fake_data_same_nb_1000_1.json")
# train4 = pd.concat([train4, tmp_train]).reset_index(drop = True)
# train4 = train4.sample(frac=0.75, random_state=777).reset_index(drop = True) #2000 * 0.75

train5 = pd.read_json(
    "../kaggle_dataset/replace/znf_nb_lzc_general_topic_2k_0219_gpt_replace.json")
train5 = train5.sample(frac=0.25, random_state=777).reset_index(
    drop=True)  # 2000 * 0.25

train6 = pd.read_json(
    "../kaggle_dataset/replace/have_label_replace_400_replace.json")
# train6 = train6.sample(frac=0.25, random_state=777).reset_index(drop = True)

train7 = pd.read_json(
    "../kaggle_dataset/replace/Fake_data_fix_prompt_500_replace.json")
# for i in range(1,4):
#     tmp_train = pd.read_json(f"../kaggle_dataset/Fake_data_fix_prompt_500_{i}.json")
#     train7 = pd.concat([train7, tmp_train]).reset_index(drop = True)

# train7 = train7.sample(frac=0.75, random_state=777).reset_index(drop = True) #2000 * 0.5

# train8 = pd.read_json("../kaggle_dataset/rewrite/rewrite_1.5k_more_name_en_name.json")
# train8 = train8.sample(frac=0.5, random_state=777).reset_index(drop = True) #1200 * 0.5

# train9 = pd.read_json("../kaggle_dataset/rewrite/rewrite_only_student_name_700.json")
# train9 = train9.sample(frac=0.25, random_state=777).reset_index(drop = True)

train10 = pd.read_json(
    "../kaggle_dataset/replace/lzc_1k_0223_newdis_newprompt_gpt_replace.json")
train10 = train10.sample(frac=0.5, random_state=777).reset_index(drop=True)

# train11 = pd.read_json("../kaggle_dataset/rewrite/rewrite_only_student_name_700_gpt.json")
# train11 = train11.sample(frac=0.75, random_state=777).reset_index(drop = True)

# train12 = pd.read_json("../kaggle_dataset/rewrite/rewrite_no_label_2k.json")
# train12 = train12.sample(frac=0.25, random_state=777).reset_index(drop = True)

# train13 = pd.read_json("../kaggle_dataset/rewrite/only_student_name_add_information_1k.json")
# train13 = train13.sample(frac=0.2, random_state=777).reset_index(drop = True)

# train14 = pd.read_json("../kaggle_dataset/rewrite/rewrite_no_label_add_1.5k.json")
# train14 = train14.sample(frac=0.1, random_state=777).reset_index(drop = True)

# train15 = pd.read_json("../kaggle_dataset/rewrite/have_label_replace_400.json")


train_df = pd.concat([train1, train2, train3, train4,
                     train5, train6, train7, train10]).reset_index(drop=True)
...
data = data.explode(["tokens", "labels"]).reset_index(drop=True)

# all_labels = sorted(list(set(chain(*[x["labels"] for x in data]))))

all_labels = sorted(list(data.labels.unique()))
...
# B-NAME_STUDENT
weights[label2id['B-NAME_STUDENT']] = weights[label2id['B-NAME_STUDENT']]
# I-NAME_STUDENT
weights[label2id['I-NAME_STUDENT']] = weights[label2id['I-NAME_STUDENT']]
# I-URL_PERSONAL
weights[label2id['I-URL_PERSONAL']] = weights[label2id['I-URL_PERSONAL']] / 200
# weights[-3] = weights[-3] / 15

# weights=[248.3889342866871,
#           240.2892951251647,
#           306.81679774572063,
#           257.0536645525018,
#           155.11764830958962,
...
def main():
    ...
    with open("./hypertuning_kfold.yaml") as file:
    ...
    # f_loss_alpha = wandb.config.focal_loss_alpha
    f_loss_gamma = wandb.config.focal_loss_gamma
    ...
    scheduler = get_polynomial_decay_schedule_with_warmup(
        optimizer,
        num_warmup_steps=int(total_steps) * 0.25,
